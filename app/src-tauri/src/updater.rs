...
//! Portunus's own desktop app.
//!
//! "Auto-update" here means auto-*checked*, one-click-confirmed -- never a
//! silent unattended swap. Replacing a running router app out from under
...
const REPO: &str = "mdostal/heimdall";
const ASSET_PATTERN: &str = "heimdall-desktop-*.zip";
...
fn current_app_bundle_path(app: &AppHandle) -> PathBuf {
    ...
    app.path()
        ...
        .unwrap_or_else(|| PathBuf::from("/Applications/Heimdall.app"))
}
...
fn prompt_and_maybe_update(app: &AppHandle, tag: String) {
    ...
    app.dialog()
        .message(format!(
            "Heimdall {tag} is available (you're running {}). Install and relaunch now?",
            app.package_info().version
        ))
        .title("Heimdall Update Available")
...
fn tempdir() -> std::io::Result<PathBuf> {
    let dir = std::env::temp_dir().join(format!("heimdall-update-{}", std::process::id()));
