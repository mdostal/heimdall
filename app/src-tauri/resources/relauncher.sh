#!/usr/bin/env bash
# Swaps the installed Heimdall.app for a freshly-downloaded one. Spawned as
...
# self-update pattern (spawn-a-helper-then-exit), adapted from Portunus's
# own real, shipped relauncher.
#
# Safety, not an afterthought: the new bundle is copied and sanity-checked
# (Info.plist + codesign) BEFORE the old one is touched at all. The actual
# swap renames the old app aside as a backup rather than deleting it
# outright -- the backup is only removed after the new app is confirmed in
# place, and is restored on any failure. A corrupted/incomplete download
# must never destroy a working install.
#
# Args: $1 = path to the downloaded new-app .zip
#       $2 = path to the currently-installed .app to replace
#       $3 = PID of the Heimdall process to wait for exit (so we don't
...
INSTALLED_APP="$2"
PARENT_PID="$3"
LOG="/tmp/heimdall-relauncher.log"
...
# HEIMDALL_RELAUNCHER_TEST_FAIL_SECOND_MV is a test-only hook (never set in
# real use) letting a test suite deterministically exercise the restore-
# from-backup path below -- this is the single highest-risk step in the
# whole flow, so it deserves a real failure-path test, not just a
# happy-path one.
if [ "${HEIMDALL_RELAUNCHER_TEST_FAIL_SECOND_MV:-}" = "1" ]; then
